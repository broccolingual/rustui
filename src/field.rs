...
pub type Field = [[block::BlockType; FIELD_WIDTH]; FIELD_HEIGHT];

pub fn get_field() -> Field {
  [[block::BlockType::default(); FIELD_WIDTH]; FIELD_HEIGHT]
}
...
pub trait FieldExt {
  ...
  fn get_block(&self, x: usize, y: usize) -> block::BlockType;
...
impl FieldExt for Field {
  ...
  fn remove_block(&mut self, block: &block::Block) {
    for pos in block.get_relative_positions() {
      ...
      if self.is_valid(x, y) {
        self[y][x] = block::BlockType::Empty; // Reset to empty
      }
    }
  }
  ...
  fn check_collision(&self, block: &block::Block) -> bool {
    for pos in block.get_relative_positions() {
      let x = pos.x as usize;
      let y = pos.y as usize;
      if !self.is_valid(x, y) || !self.get_block(x, y).is_empty() {
        return true;
      }
    }
    false
  ...
  fn is_gameover(&self) -> bool {
    for x in 0..FIELD_WIDTH {
      if !self[FIELD_MARGIN][x].is_empty() {
  ...
  fn check_complete_line(&self, y: usize) -> bool {
    self[y].iter().all(|&block| !block.is_empty())
  }
  ...
  fn clear_lines(&mut self) -> u8 {
    ...
    self[0] = [block::BlockType::Empty; FIELD_WIDTH]; // Clear the top line
    cleared
  }
}
