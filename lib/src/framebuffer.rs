...
impl Framebuffer {
  ...
  pub fn refresh(&mut self, back_fb: &Framebuffer) -> io::Result<()> {
    if self.height != back_fb.height || self.width != back_fb.width {
      return Err(io::Error::new(io::ErrorKind::InvalidInput, "Framebuffer sizes do not match"));
    }
    ...
    for y in 0..self.height {
      for x in 0..self.width {
        ...
        if front != back { // フロントバッファのCellとバックバッファのCellが異なる場合のみ描画
          if prev_attr != Some(&back.style) { // 属性が異なる場合のみ属性を更新
